...
function gadget_mgr.load(path,id) -- carga el codigo y lo inserta al shell
	...
	if not files.exists(root) then box.new("Gadget Error","Main.lua not found."); return end -- comprobamos exista antes que nada
	local numofgadgets = gadget_mgr.open -- numero de gadget abiertas o cargadas
	if numofgadgets >= max_open_gadget then box.new("Gadget Error","Maximum number of Gadgets reached."); return end
...
function gadget_mgr.create(id_name) -- regista la gadget esto en si la llama a load
	...
	if not not_err then -- hay un error
		box.new("Gadget Error"..gadget_mgr.focus, msg)
		return
	end
end

function gadget_mgr.loadall()
	local listaofgadgets = ini.load("config/gadget.ini")--files.listdirs(root_to_gadgets)
